(function($, _) { // wrap in anonymous function to not show some helper variables
   ...
   var translations = {
     en: {
       trash_label: 'Déplacer les lignes de code',
       solution_label: 'Construire votre solution ici',
       order: function() {
         return "Les fragments de code de votre programme sont faux, ou dans un mauvais ordre. Déplacez, supprimez ou remplacez les fragments surlignés.";},
       lines_missing: function() {
         return "Il manque des fragments dans votre code.";},
       lines_too_many: function() {
         return "Il y a trop de fragments dans votre code.";},
       no_matching: function(lineNro) {
         return "Le fragment (" + lineNro + ") surligné n'est pas correctement indenté."; },
       no_matching_open: function(lineNro, block) {
         return "Le " + block + " qui se termine ligne " + lineNro + " ne débute nulle part."; },
       no_matching_close: function(lineNro, block) {
         return "Le bloc " + block + " défini à la ligne " + lineNro + " ne s'est pas terminé correctement.";
       },
       block_close_mismatch: function(closeLine, closeBlock, openLine, inBlock) {
         return "Impossible de terminer le bloc " + closeBlock + " à la ligne " + closeLine + " en étant toujours à l'intérieur du bloc " + inBlock + " qui démarre à la ligne " + openLine;
       },
       block_structure: function(lineNro) { return "Le fragment surligné " + lineNro + " appartient au mauvais bloc (problème d'indentation)."; },
       unittest_error: function(errormsg) {
         return "<span class='msg'>Erreur à l'exécution du programme</span><br/> <span class='errormsg'>" + errormsg + "</span>";
       },
       unittest_output_assertion: function(expected, actual) {
        return "Sortie attendue : <span class='expected output'>" + expected + "</span>" +
              "Sortie renvoyée par votre programme : <span class='actual output'>" + actual + "</span>";
       },
  ...
  UnitTestGrader.prototype.grade = function(options, studentcode) {
    ...
    for (var i = 0, l = result.length; i < l; i++) {
      ...
      if (res.status === "error") { // errors in execution
        ...
        feedbackHtml += 'Attendu <span class="expected">' + res.expected +
